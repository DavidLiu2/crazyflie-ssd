...
void Convolution2(
  ...
  for(iter=0; iter < total_tiles; iter++) {
    // check if last in any dimension
      ...
      x_tile_size_h   = (_i_h_load+1 == 5)   ? 16 : 16;
      x_tile_size_w   = (_i_w_load+1 == 2)   ? 24 : 56;
      ...
      if(_i_w_load > 0)
        pad_offset_w = 0;
      y_tile_size_h   = (_i_h_load+1 == 5)   ? 16 : 16;
      y_tile_size_w   = (_i_w_load+1 == 2)   ? 24 : 56;
      ...
      {
        DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 16, 56, 8, 80, 8,  0, 0,0, pad_offset_h, pad_offset_w, 0, 8);
      ...
      {
        DMA_copy_W.ext = dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 8, 1*1, 8, 1*1, 8, 0,0,0,0,0,0, 8);
        DMA_copy_W.loc = (l1_buffer + 35856);
    ...
    W = (uint8_t *) (l1_buffer + 35856);
    y = (uint8_t *) (l1_buffer + 7176);
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    pi_cl_team_barrier(0);
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_load, _i_w_load, _i_nof_load, 16, 56, 8, 80, 8, 0, 0, 0, 0, 0, 0, 32);
      DMA_copy_y.loc = (l1_buffer + 7176);
