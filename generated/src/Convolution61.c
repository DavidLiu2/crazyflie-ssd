...
void Convolution61(
  ...
  for(iter=0; iter < total_tiles; iter++) {
    // check if last in any dimension
      ...
      {
        DMA_copy_W.ext = dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 32, 1*1, 96, 1*1, 96, 0,0,0,0,0,0, 8);
        DMA_copy_W.loc = (l1_buffer + 5616);
    ...
    pi_cl_team_barrier(0);
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_load, _i_w_load, _i_nof_load, 5, 5, 32, 5, 32, 0, 0, 0, 0, 0, 0, 32);
