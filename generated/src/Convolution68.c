...
void Convolution68(
  ...
  DMA_copy_bias.loc = (uint32_t) (l1_buffer + 29992);
  ...
  for(iter=0; iter < total_tiles; iter++) {
    // check if last in any dimension
      ...
      x_tile_size_h   = (_i_h_load+1 == 1)   ? 20 : 20;
      x_tile_size_w   = (_i_w_load+1 == 1)   ? 20 : 20;
      ...
      if(_i_w_load > 0)
        pad_offset_w = 1;
      y_tile_size_h   = (_i_h_load+1 == 1)   ? 20 : 20;
      ...
      {
        DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 20, 20, 8, 20, 8,  2, 2,0, pad_offset_h, pad_offset_w, 0, 8);
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        DMA_copy_W.loc = (l1_buffer + 28816);
    ...
    b = (uint8_t *) (l1_buffer + 29992 + _i_nof_load*64);
    W = (uint8_t *) (l1_buffer + 28816);
    y = (uint8_t *) (l1_buffer + 3208);
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    pi_cl_team_barrier(0);
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_load, _i_w_load, _i_nof_load, 20, 20, 16, 20, 16, 0, 0, 0, 0, 0, 0, 32);
      DMA_copy_y.loc = (l1_buffer + 3208);
