...
void Convolution65(
  ...
  DMA_copy_x.stride_1d = 16;
  DMA_copy_x.dir = 1;
  DMA_copy_x.tid = dory_dma_channel;
  
  DMA_copy_W.hwc_to_chw = 0;
  DMA_copy_W.stride_2d = 144;
  DMA_copy_W.stride_1d = 16;
  ...
  ////////////////////////////
  DMA_copy_bias.ext = (uint32_t) l2_W+4608;
  DMA_copy_bias.loc = (uint32_t) (l1_buffer + 19048);
  ...
  for(iter=0; iter < total_tiles; iter++) {
    // check if last in any dimension
      x_tile_size_nif = (_i_nif_load+1 == 1) ? 16 : 16;
      x_tile_size_h   = (_i_h_load+1 == 1)   ? 10 : 10;
      x_tile_size_w   = (_i_w_load+1 == 1)   ? 10 : 10;
      x_tile_size_byte = x_tile_size_nif*x_tile_size_h*x_tile_size_w*8/8;
      x_length_nif_byte = (_i_nif_load+1 == 1)   ? 16 : 16;
      ...
      if(_i_w_load > 0)
        pad_offset_w = 1;
      y_tile_size_h   = (_i_h_load+1 == 1)   ? 10 : 10;
      y_tile_size_w   = (_i_w_load+1 == 1)   ? 10 : 10;
      ...
      W_tile_size_nif = (_i_nif_load+1 == 1) ? 16 : 16;
      W_tile_size_byte = W_tile_size_nof*W_tile_size_nif*8*3*3/8;
      W_length_nif_byte = (_i_nif_load+1 == 1) ? 16 : 16;
      // transfer of next input tile in double buffering
      if (_i_nif_load!=_i_nif_exec || _i_w_load!=_i_w_exec || _i_h_load!=_i_h_exec)
      {
        DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 10, 10, 16, 10, 16,  2, 2,0, pad_offset_h, pad_offset_w, 0, 8);
      ...
      {
        DMA_copy_W.ext = dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 32, 3*3, 16, 3*3, 16, 0,0,0,0,0,0, 8);
        DMA_copy_W.loc = (l1_buffer + 14416);
    ...
    b = (uint8_t *) (l1_buffer + 19048 + _i_nof_load*128);
    W = (uint8_t *) (l1_buffer + 14416);
    y = (uint8_t *) (l1_buffer + 1608);
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    pi_cl_team_barrier(0);
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_load, _i_w_load, _i_nof_load, 10, 10, 32, 10, 32, 0, 0, 0, 0, 0, 0, 32);
      DMA_copy_y.loc = (l1_buffer + 1608);
