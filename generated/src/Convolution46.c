...
void Convolution46(
  ...
  DMA_copy_W.stride_2d = 96;
  DMA_copy_W.stride_1d = 96;
  ...
  for(iter=0; iter < total_tiles; iter++) {
    // check if last in any dimension
      ...
      W_length_nif_byte = (_i_nif_load+1 == 1) ? 96 : 96;
      // transfer of next input tile in double buffering
      if (_i_nif_load!=_i_nif_exec || _i_w_load!=_i_w_exec || _i_h_load!=_i_h_exec)
      {
        DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 10, 10, 96, 10, 96,  0, 0,0, pad_offset_h, pad_offset_w, 0, 8);
